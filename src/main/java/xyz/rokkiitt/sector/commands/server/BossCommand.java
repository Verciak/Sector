    ...
    public boolean onCommand(final Player p, final String[] args) {
        ...
        if(args[0].contains("list")){
            BossManager.users.forEach(boss -> {
                p.sendMessage(Util.fixColor("&cBoss &8- &6" + boss.getAuthor()));
            });
        }
        if(args[0].contains("create")){
            ...
            p.sendTitle(Util.fixColor("&cBOSS"), Util.fixColor("&7X: &6" + p.getFloorX() + " &7Y: &6" + p.getFloorY() + " &7Z: &6" + p.getFloorZ()), 30, 80, 30);
