...
public class modifySponsor extends DoubleChestFakeInventory {
    public modifySponsor(Player p) {
        super(null, Util.fixColor("&6Modyfikujesz kit sponsor"));
        this.state = State.MAIN;
        this.drops = ConcurrentHashMap.newKeySet();
        this.holder = (InventoryHolder)p;
        for (KitData d : KitManager.getSponsor()) {
                ...
                if (e.getAction().getSlot() == 31) {
                    this.drops.forEach(s -> {
                        if (s.equals(this.selected)) {
                            ...
                            s.what.setCount(1);
                            Main.getDatabase().addQueue("DELETE FROM `sponsorkit` WHERE `drop` = '" + JsonStream.serialize(drop) + "'");

                            this.state = modifySponsor.State.MAIN;
                            sendGui();
                        }
                    });
    ...
    public void onClose(Player p) {
        ...
        if (this.drops.isEmpty()) {
            ...
            Main.getDatabase().addQueue("TRUNCATE sponsorkit;");
            ...
            this.drops.forEach(s -> {
                ...
                pa.data = data;
                Main.getDatabase().addQueue("INSERT INTO sponsorkit(`id`, `drop`, `item`) VALUES (NULL, '" + pa.data + "', '"+wath+"')");
            });
        }
        super.onClose(p);
        Server.getInstance().getScheduler().scheduleDelayedTask(Main.getPlugin(), () -> KitManager.load("sponsor"), 100);
