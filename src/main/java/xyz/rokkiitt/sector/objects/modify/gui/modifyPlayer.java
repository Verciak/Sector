...
public class modifyPlayer extends DoubleChestFakeInventory {
    public modifyPlayer(Player p) {
        super(null, Util.fixColor("&6Modyfikujesz kit gracz"));
        this.state = State.MAIN;
        this.drops = ConcurrentHashMap.newKeySet();
        this.holder = (InventoryHolder)p;
        for (KitData d : KitManager.getPlayer()) {
                ...
                if (e.getAction().getSlot() == 31) {
                    this.drops.forEach(s -> {
                        if (s.equals(this.selected)) {
                            ...
                            s.what.setCount(1);
                            Main.getDatabase().addQueue("DELETE FROM `playerkit` WHERE `drop` = '" + JsonStream.serialize(drop) + "'");
    ...
    public void onClose(Player p) {
        ...
        if (this.drops.isEmpty()) {
            ...
            Main.getDatabase().addQueue("TRUNCATE playerkit;");
            ...
            this.drops.forEach(s -> {
                ...
                pa.data = data;
                Main.getDatabase().addQueue("INSERT INTO playerkit(`id`, `drop`, `item`) VALUES (NULL, '" + pa.data + "', '"+wath+"')");
            });
        }
        super.onClose(p);
        Server.getInstance().getScheduler().scheduleDelayedTask(Main.getPlugin(), () -> KitManager.load("player"), 100);
