...
public class modifySvip extends DoubleChestFakeInventory {
    public modifySvip(Player p) {
        super(null, Util.fixColor("&6Modyfikujesz kit svip"));
        this.state = State.MAIN;
        this.drops = ConcurrentHashMap.newKeySet();
        this.holder = (InventoryHolder)p;
        for (KitData d : KitManager.getSvip()) {
                ...
                if (e.getAction().getSlot() == 31) {
                    this.drops.forEach(s -> {
                        if (s.equals(this.selected)) {
                            ...
                            s.what.setCount(1);
                            Main.getDatabase().addQueue("DELETE FROM `svipkit` WHERE `drop` = '" + JsonStream.serialize(drop) + "'");

                            this.state = modifySvip.State.MAIN;
                            sendGui();
                        }
                    });
    ...
    public void onClose(Player p) {
        ...
        if (this.drops.isEmpty()) {
            ...
            Main.getDatabase().addQueue("TRUNCATE svipkit;");
            ...
            this.drops.forEach(s -> {
                ...
                pa.data = data;
                Main.getDatabase().addQueue("INSERT INTO svipkit(`id`, `drop`, `item`) VALUES (NULL, '" + pa.data + "', '"+wath+"')");
            });
        }
        super.onClose(p);
        Server.getInstance().getScheduler().scheduleDelayedTask(Main.getPlugin(), () -> KitManager.load("svip"), 100);
    }
