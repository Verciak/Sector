...
public class modifyPandora extends DoubleChestFakeInventory {
    public modifyPandora(Player p) {
        super(null, Util.fixColor("&6Modyfikujesz pandore"));
        this.state = State.MAIN;
        this.drops = ConcurrentHashMap.newKeySet();
        this.holder = (InventoryHolder)p;
        for (Pandora d : PandoraManager.getItems()) {
                ...
                if (e.getAction().getSlot() == 31) {
                    this.drops.forEach(s -> {
                        if (s.equals(this.selected)) {
                            ...
                            Main.getDatabase().addQueue("DELETE FROM `pandoradrops` WHERE `drop` = '" + JsonStream.serialize(drop) + "'");
    ...
    public void onClose(Player p) {
        ...
        if (this.drops.isEmpty()) {
            ...
            Main.getDatabase().addQueue("TRUNCATE pandoradrops;");
            this.drops.forEach(s -> {
                SerializedPandoraItem drop = new SerializedPandoraItem();
                drop.chance = s.chance;
                drop.guislot = s.guislot;
                ...
                pa.data = data;
                Main.getDatabase().addQueue("INSERT INTO pandoradrops(`id`, `drop`, `item`) VALUES (NULL, '" + pa.data + "', '"+wath+"')");
            });
        }
        super.onClose(p);
        Server.getInstance().getScheduler().scheduleDelayedTask(Main.getPlugin(), () -> PandoraManager.load(), 100);
